...
def set_gui_logger(func):
    """La GUI inyecta aquí su función de log (App._log) y se propaga a submódulos."""
    global _gui_logger
    _gui_logger = func
    # Propaga al submódulo de bateadores si lo expone
    try:
        if hasattr(bt, "set_gui_logger"):
            bt.set_gui_logger(func)
    except Exception:
        pass
    # Propaga al submódulo de pitchers si lo expone
    try:
        if hasattr(pt, "set_gui_logger"):
            pt.set_gui_logger(func)
    except Exception:
        pass

# ---- Rutas/constantes necesarias ----
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_BASE_ROOT = os.path.join(SCRIPT_DIR, 'BaseballBatterPitcherReports')

DIR_BATTER = os.path.join(_BASE_ROOT, 'Batter')
DIR_PITCHER = os.path.join(_BASE_ROOT, 'Pitcher')
DIR_HOMEPLATE = os.path.join(_BASE_ROOT, 'img')

os.makedirs(DIR_BATTER, exist_ok=True)
os.makedirs(DIR_PITCHER, exist_ok=True)
os.makedirs(DIR_HOMEPLATE, exist_ok=True)

# Propaga rutas a submódulos (sobrescribe valores viejos)
try:
    bt.DIR_TEMP = DIR_BATTER
    bt.DIR_HOMEPLATE = DIR_HOMEPLATE
except Exception:
    pass

try:
    pt.DIR_TEMP = DIR_PITCHER
    pt.DIR_HOMEPLATE = DIR_HOMEPLATE
except Exception:
    pass
...
def clean_directory(base_subdir: str, base_path: Optional[str] = None) -> None:
    ...
    logger.info(f"clean_directory: Limpiando directorio {base_subdir}")
    base_path = base_path or os.path.join(_BASE_ROOT, base_subdir)
    if not os.path.isdir(base_path):
        logger.debug(f"clean_directory: Directorio no existe: {base_path}")
        return
    ...
    for root, _, files in os.walk(base_path, topdown=False):
        for file in files:
            ...
            if is_png and (base_subdir == "Pitcher" or is_batter_non_spray):
                try:
                    logger.debug(f"clean_directory: Eliminando archivo: {os.path.join(root, file)}")
                    os.remove(os.path.join(root, file))
                except OSError:
                    logger.warning(f"No se pudo eliminar: {os.path.join(root, file)}")
                    logger.warning(f"clean_directory: Error de OS al eliminar {os.path.join(root, file)}")
        try:
            if not os.listdir(root):
                logger.debug(f"clean_directory: Eliminando directorio vacío: {root}")
                os.rmdir(root)
        except OSError:
            logger.warning(f"clean_directory: Error de OS al eliminar directorio {root}")
            pass

    logger.info(f"clean_directory: Limpieza de {base_subdir} finalizada")
