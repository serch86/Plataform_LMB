...
# from spray_charts_f import *  # ← Comentado: archivo ausente, se reemplaza con stubs debajo
import dataframe_image as dfi

# === STUBS que reemplazan a spray_charts_f (gráficas vacías para no romper el flujo) ===
...
# === Reemplazo del stub: genera un PDF real a partir de los PNG ya creados ===
def create_report(batter_name, team, dir_images, dict_short, n_left, n_right, dir_homeplate=None):
    """
    Ensambla un PDF simple con los PNG existentes en dir_images.
    Ordena por nombre de archivo para mantener consistencia.
    ...
    # Recolectar imágenes PNG
    if not os.path.isdir(dir_images):
        return out_path
    imgs = [f for f in os.listdir(dir_images) if f.lower().endswith(".png")]
    if not imgs:
        # Si no hay imágenes, crear PDF con portada básica
        pdf = FPDF()
        pdf.add_page()
    ...
    imgs.sort()  # orden alfabético estable
...
def _external_load_batter_sz():
    query = '''
        SELECT BatterId, Batter, strikeZoneTop, strikeZoneBottom
        FROM baseballlmb.trackman_db.batters_sz_2025
    '''
    df_bat = client.query(query).result().to_dataframe(bqstorage_client=bqstorageclient)
    return df_bat
"""
# ----------------------------------------------------------------------
# CONSTANTES
...
# ----------------------------------------------------------------------
# ESTILOS Y EXPORTACIÓN DE TABLAS/FIGURAS
# ----------------------------------------------------------------------
...
def create_report_full(batter_name,
    ...
    """
    Genera el reporte PDF del bateador.
    """
