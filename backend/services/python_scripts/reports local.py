...
"""
# Dependencias para BigQuery (opcional)
...
def load_trackman_dataframe(query: str = BQ_DEFAULT_QUERY,
                            local_file: Optional[str] = None) -> pd.DataFrame:
    """
    Carga datos de Trackman.
    - MODO LOCAL (activo): lee CSV/Parquet para pruebas.
    - MODO BIGQUERY (comentado): consulta datos reales.
    """
    # -------------------------------
    # LOCAL
    # -------------------------------
    local_path = local_file or os.path.join(os.path.dirname(__file__), "datos_local_trackman.csv")
    ...
    if df.empty:
        raise RuntimeError("No hay registros en la consulta de BigQuery.")
...
def run_batter_reports(df: pd.DataFrame,
    ...
    for nombre in nombres:
        try:
            # Se asume que bt.create_report_full puede devolver ruta; si no, ignora.
            path = bt.create_report_full(nombre, df_stats, df_games, dict_baseball_teams_short,
                                         work_dir=work_dir)  # si la función no acepta, Python ignorará kwargs? No.
            if path:
                artefactos.append(path)
            generated += 1
        except TypeError:
            # Versión sin work_dir
            try:
                path = bt.create_report_full(nombre, df_stats, df_games, dict_baseball_teams_short)
                if path:
                    artefactos.append(path)
...
def run_pitcher_reports(df: pd.DataFrame,
    ...
    cond2 = pd.Series(True, index=df_table.index)  # en lugar de any(axis=1)
...
def main(df: Optional[pd.DataFrame] = None,
    ...
    Si df es None, carga datos vía load_trackman_dataframe(query, local_file).
    """
    if df is None:
        df = load_trackman_dataframe(query=query, local_file=local_file)
