...
class Tools:
    ...
    def extract_table_sections_from_excel(self, filepath: str) -> List[pd.DataFrame]:
        ...
        for i, row in df_raw.iterrows():
            if self.is_header_row(row) or self.is_section_title(row):
                title = next((str(cell).strip().lower()
                              for cell in row if str(cell).strip().lower() in ['catchers', 'pitchers', 'infielders', 'outfielders']), None)
                start_rows.append(i)
        ...
        for (start, end), title in zip(zip_longest(start_rows, start_rows[1:]), title_rows):
            ...
            section.columns = [str(c).lower() if pd.notna(c) else f'unnamed_{i}' for i, c in enumerate(section.iloc[0])]
            ...
            try:
                processed_section = self._extraer_nombres_y_posicion(section, title)
    ...
    def cargar_excel(self, filepath: str) -> pd.DataFrame:
        ...
        try:
            ...
            df_data.columns = [str(c).lower() if pd.notna(c) else f'unnamed_{i}' for i, c in enumerate(df_data.iloc[0])]
            ...
            return pd.concat(secciones, ignore_index=True)
    ...
    def generar_resumen(self, df: pd.DataFrame) -> Dict:
        total = len(df)
        posiciones = df['position'].str.strip().str.lower().value_counts().to_dict()
        return {
            "total_jugadores": total,
            "posiciones": posiciones
        }

    def procesar_archivo(self, filepath: str) -> Dict:
        ...
        try:
            ...
            nombres_trackman_csv = os.path.join(os.path.dirname(__file__), "nombres_trackman.csv")
            nombres_trackman = self.cargar_nombres_trackman(nombres_trackman_csv)

            nombres_archivo = tabla['raw_name'].dropna().tolist()
            coincidencias = self.encontrar_similares(nombres_archivo, nombres_trackman, umbral=90)
            ...
            return {
                base: {
                    "error": f"Error al procesar archivo: {type(e).__name__} - {e}"
                }
            }
